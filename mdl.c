 ...
 *
 * TODO: It doesn't do much with the timing information in textures
...
MDL_MESH *mdl_load(const char *filename) {
	...
	M->tex_times = fx_calloc(M->num_textures, sizeof *M->tex_times);
	if(!M->tex_times) {
		fx_error("MDL: out of memory\n");
		return NULL;
	}
	M->textures = fx_calloc(M->num_textures, sizeof *M->textures);
	if(!M->textures) {
		fx_error("MDL: out of memory\n");
		return NULL;
	}
	...
	if(group) {
		if(fread(M->tex_times, sizeof *M->tex_times, M->num_textures, f) != (unsigned)M->num_textures) {
			fx_error("MDL: unable to read tex_times: %s\n", strerror(errno));
			return NULL;
		}
