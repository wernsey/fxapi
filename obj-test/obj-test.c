...
int init_thing(int argc, char *argv[]) {
	...
	if(argc < 2) {
		error("usage: %s model.obj [texture]", argv[0]);
		return 0;
	}
	...
	// bm_clip(screen, 20, 20, screen->w - 20, screen->h - 20);
...
void update_screen(int64_t ms) {
	...
	// fx_fog(0);
	fx_all_lighting(1);
	fx_set_ambient(0.2, 0.2, 0.2);

	fx_light_enable(0);
	fx_set_diffuse_direction(0, -0.5, -1.0, -0.5);
	fx_set_diffuse_color(0, 0.8, 0.8, 0.8);
